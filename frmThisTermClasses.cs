...
{
    public partial class frmThisTermClasses : Form
    {
        public frmThisTermClasses()
        {
            InitializeComponent();
        }
        List<View_StudentClassRoomMapping> classes;
        private void frmThisTermClasses_Load(object sender, EventArgs e)
        {
            University_DBEntities db = new University_DBEntities();
            int openterm = Convert.ToInt32(clsPublicVariable.openTerm);
            classes = db.View_StudentClassRoomMapping.Where(c => c.StudentId == clsPublicVariable.User.UserId && c.Year == clsPublicVariable.Year && c.Term == openterm).ToList();
            Fillform();
        }
